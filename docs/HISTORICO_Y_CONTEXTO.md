...
| 2026-09-24 | **CI en verde (compañero DevOps tenía razón en las reglas)**: el pipeline fallaba por causas reales, no por CI mal configurado. (1) `schema.sql:149` traía `fecha_publicacion` pegada dentro de un comentario → PG fresco del CI sin esa columna → 25 tests en 503. (2) Coverage FE branches 68.3% < 70%: tests honestos nuevos (Recuperar/Restablecer/Legal/AuthCallback-errores, ramas utils) + eliminar `nombresServicios` muerto → 71.2%. (3) Coverage BE 59% < 70%: el portal de TestClient corre en otro hilo y coverage no lo mide → suite `test_ci_fallbacks` (27, fail-closed/mock honestos) + invocación directa en hilo principal `test_ci_directo_base/api` (servicios, repos, endpoints) + unidades puras → 82.2%. Fixes reales hallados: `listar/revisar reportes` y `ver_auditoria` daban 500 crudo (ahora 503 fail-closed). Alineada BD dev (`pg_trgm`+`unaccent` que le faltaban vs schema.sql) y `.gitignore` ignora cobertura | `pytest 363 passed`, `vitest 380 passed`, `build` OK; sin push a origin |
| 2026-09-25 | **Sprint M1-M5 Panel+Tipos+Perfil+Filtros+Telegram (aditivo, sin push)**: M1 `MetricasOut` += `inmuebles_con_reportes` (COUNT DISTINCT PENDIENTE, `idx_reportes_pub_estado`, `reportes_activos` intacto) + tarjeta Pendientes con tooltip denuncias-vs-inmuebles + badge rojo `reportes_pendientes` + breadcrumbs `Inicio › Panel Admin › [Pestaña]` (`BreadcrumbsAdmin`, móvil `← Volver al Panel`) + auditoría human-readable (`mensaje_legible`, batch `usuario_id→email` 1 query, link `#ID` si existe / texto si eliminada). M2 mig 013 `housing_types` (6 slugs, FK RESTRICT, seed+schema) + validador Pydantic/Query dinámico caché 5min (sin `pattern=`) + sync `trust`/`search` + `/config-publica.tipos_vivienda` + 6 componentes FE con `useTiposVivienda` fallback + CRUD admin auditado + `InfoTooltip` en Publicar/Filtros. M3 `POST/DELETE /api/auth/avatar` (cualquier rol, 5MB, misma txn, sin tocar `/upload/una`) + `AvatarPerfil` (hover Edit, sheet Ver/Cambiar/Quitar, sin URL) + checklist 20+20+20+15+15+10=100 + `Tus Preferencias` solo `filtros.*` (🐾/🌙/🚬). M4 `MasFiltrosModal` desktop (primarios en barra, secundarios en drawer, `contarAvanzados`, sheet móvil intacto). M5 `TELEGRAM_BOT_USERNAME` + `POST /telegram/vincular-inicio` (HMAC 1 uso 5min, `bot_url` directo) + bugfix OTP solo DM con `telegram_chat_id` (mig 014, nunca canales; fallback email con canal correcto) + `TelegramVincular` + `telegram_vinculado` en perfil | `pytest 370 passed +1 skipped`, `vitest 400 passed`, `build` OK; FE branches 70.64% / BE >70% (vía suites CI); sin push a origin (commit local). Aplicar en prod: 013_housing_types + 014_telegram_chat_id |
| 2026-09-26 | **Auditoría Staff (72/100) + Bloques 1-3 por sprints locales (sin push)**: informe read-only con 17 hallazgos (3 ALTO). **B1** (`f4a9ae7`): Casa3D vía `React.lazy` + gate `useMediaQuery(>=1024px)` (three ~736 kB ya no baja en móvil) + throttle avatar 10/h y vincular 5/15min (memoria+PG, tras validar forma) + `borrar_local_si_huerfano` (reemplazo/DELETE/fracaso-DB). **B2** (`13f0c17`): retry solo GET/HEAD/OPTIONS salvo `Idempotency-Key` + `conIdempotencia()` en Publicar + replay en POST crear (UNIQUE cierra carrera, TTL 24h, espejo mock) + Telegram en PG (mig 015 `telegram_vinculos` + 016 `idempotency_keys`, `validar_token_vinculo_db` con FOR UPDATE; sync intacto). **B3** (`2a43dc2`): `getEtiquetaTipo` fuente única (wrappers `humanizar*` + legacy APARTAMENTO intactos) + `useFocusTrap` en 8 diálogos + cleanup en 5 timeouts (MapPicker ya lo tenía). Cazado por suites: reseed reinicia identities (cuotas en memoria se aíslan por fixture), `Header` default llega como objeto en invocación directa, hook con fetch rompe mocks Once (anteponer mock config + `limpiarConfigCache`), índices nuevos exigen registro `POST_*` en `test_indices_sync`. Rechazos fundados: custom selects globales (picker nativo es mejor en móvil), servicios agua/luz/gas y proximidad (no existen en BD/API; placebo prohibido), focus-trap total inmediato, renombrar ROL_LABEL. Aplicar en prod: 015_telegram_vinculos + 016_idempotency_keys | `pytest 383 passed +1 skipped` (gate CI 76.29%), `vitest 510 passed` (branches 74.78%), `build` OK; sin push a origin (3 commits locales) |
