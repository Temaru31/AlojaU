    ...
    <div className="fixed inset-0 z-[2000] bg-black/90 flex flex-col" role="dialog" aria-modal="true" aria-label="Visor de fotos">
      ...
      <div className="relative z-10 flex items-center justify-between px-3 sm:px-4 py-3 text-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onClose} aria-label="Cerrar visor" className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-lg">×</button>
        </div>
      </div>

      {/* Imagen */}
      <div className="relative z-10 flex-1 flex items-center justify-center p-2 sm:p-4 min-h-0">
        <button type="button" onClick={goPrev} aria-label="Anterior" className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center border border-white/20">‹</button>
        ...
        <button type="button" onClick={goNext} aria-label="Siguiente" className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center border border-white/20">›</button>
      </div>
      ...
      <div className="relative z-10 px-2 sm:px-4 pb-3 sm:pb-4">
        <div className="flex gap-2 overflow-x-auto justify-center py-2 scrollbar-thin">
          {fotos.map((f,i)=> (
            <button type="button" key={i} onClick={()=> setIndex(i)} aria-label={`Ver foto ${i+1}`} className={`shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-lg overflow-hidden border-2 bg-neutral-800 ${i===index ? 'border-white' : 'border-transparent opacity-60 hover:opacity-100'}`}>
      ...
      {/* Click fuera cierra */}
      <button type="button" aria-label="Cerrar al hacer clic fuera" onClick={onClose} className="absolute inset-0 z-0" tabIndex={-1} />
    </div>,
    document.body
  )
}
